...
main_menu.add('✔️ Подписаться на канал')
main_menu.add('👤 Профиль', '👥Партнёрская программа')
main_menu.add('💎Вывод','⚙️Тех.Поддержка')
main_menu.add('🎊Промокоди')
...
payments_cancel = KeyboardButton("/Отмена")

greet_kb = ReplyKeyboardMarkup(resize_keyboard = True)
greet_kb.add(payments_cancel)
...
@dp.message_handler(lambda m: m.from_user.id in admins, commands = ['add'])
...
@dp.message_handler(lambda message: message.text == '⚙️Тех.Поддержка' and user_banned(message.from_user.id) == False)
async def support(m: types.Message):
    await m.reply("⛔️ Техническая поддержка: @Tex_freefiretop2020", reply = False )


@dp.message_handler(lambda message: message.text == '💎Вывод' and user_banned(message.from_user.id) == False)
async def enter_payments(m: types.Message):
    await m.answer("Привет!", reply_markup=greet_kb)
    await Payments.Q1.set()
    await m.answer("💰Минимальная сумма для вывода 100 💎\n\n⚠️Для вывода укажите свой ID FREE FIRE")
...
@dp.message_handler(state=Payments.Q1)
async def answer_q1(m: types.Message, state: FSMContext):
    answer = m.text
    await state.update_data(amswer1=answer)
    await bot.forward_message(chat_id, m.from_user.id, m.message_id)
    await m.answer("Сколько алмазов хотите вывести?")
    await Payments.next()

@dp.message_handler(state=Payments.Q2)
async def answer_q1(m: types.Message, state: FSMContext):
    answer2 = m.text
    data = await state.get_data()
    answer1 = data.get("answer1")
    await bot.forward_message(chat_id, m.from_user.id, m.message_id)
    await m.answer("✅Заявка успешно одобрена\n\n⏰Ожидание вывода от 30 минут до 48ч.", reply_markup = main_menu)
    await state.finish()
    
@dp.message_handler(lambda message: message.text == '🎊Промокоди' and user_banned(message.from_user.id) == False)
async def stat_message(m: types.Message):
    await m.reply("Находится в разработке!", reply = False )    
