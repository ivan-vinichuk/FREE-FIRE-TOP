...
#START = f'🙋‍♂ Добро пожаловать в бот "💎". Я создан для того, чтобы помогать тебе получать взаимных подписчиков на твой канал. Перед началом использования бота, обязательно прочитай небольшую [инструкцию по использованию бота, а также правила]({LINK_TO_INTRODUCTION_AND_RULES})'
START = f'🙋‍♂ Добро пожаловать в бот .\nЗа регистрацию тебе начислено бонус 50 💎!'

UPDATE = 'Привет еще раз!'

LITTLE_SUBCOIN_1 = f'❗️Для раскрутки вашего канала у вас должно быть минимум {LITTLE_SUBCOIN_TO_GET_SUBS} 💎!'

LITTLE_SUBCOIN_2 = '😳 Недостаточно 💎!'
...
2) _Пришлите сюда chat_id этого канала. Например: -1001334265798'''
...
def PARTNER_PROGRAM(username_of_bot, user_id, ref_count):
    message = f'🤩 Приглашайте в бота друзей и знакомых по своей реферальной ссылке и получайте по 1 💎 за каждого.\n👥 Вы уже пригласили: {ref_count}\n👣 *Ваша реферальная ссылка:* https://t.me/{username_of_bot}?start={user_id}'
    return message
