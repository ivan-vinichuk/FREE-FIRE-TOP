...
def add_user_to_db(id, **ref_father):
    if ref_father:
        ...
        conn.execute('''INSERT INTO users(id, ref_father, balance, referals, alltime_subs, fine_count, alltime_get_subs) VALUES(?,?,?,?,?,?,?)''', (id, ref_father, 50,str([]), 0, 0, 0))
        ...
        conn.execute('''INSERT INTO users(id, ref_father, balance, referals, alltime_subs, fine_count, alltime_get_subs) VALUES(?,?,?,?,?,?,?)''', (id, 0, 50, str([]), 0, 0, 0))
        conn.commit()
...
def uban_user(id, decision):
    ...
    if decision == 1:
        count = count.fetchall()[0][0]
        if count == 1:
            conn.execute('''DELETE FROM black_list WHERE id = ?''', (id,))
            conn.commit()
